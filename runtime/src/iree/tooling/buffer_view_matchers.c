...
// HACK: this should be placed on the equality struct.
#define IREE_HAL_F16_THRESHOLD 0.001f
#define IREE_HAL_F32_THRESHOLD 0.0001f
#define IREE_HAL_F64_THRESHOLD 0.0001
...
static bool iree_hal_compare_strided_elements_approximate_absolute_f16(
  ...
  for (iree_host_size_t i = 0; i < element_count; ++i) {
    if (fabsf(iree_math_f16_to_f32(*expected_ptr) -
              iree_math_f16_to_f32(*actual_ptr)) > IREE_HAL_F16_THRESHOLD) {
      *out_index = i;
      return false;
    }
...
static bool iree_hal_compare_strided_elements_approximate_absolute_f32(
    ...
    iree_host_size_t* out_index) {
  for (iree_host_size_t i = 0; i < element_count; ++i) {
    if (fabsf(*expected_ptr - *actual_ptr) > IREE_HAL_F32_THRESHOLD) {
      *out_index = i;
      return false;
    }
    expected_ptr += expected_stride;
...
static bool iree_hal_compare_strided_elements_approximate_absolute_f64(
    ...
    iree_host_size_t* out_index) {
  for (iree_host_size_t i = 0; i < element_count; ++i) {
    if (fabs(*expected_ptr - *actual_ptr) > IREE_HAL_F64_THRESHOLD) {
      *out_index = i;
      return false;
    }
    expected_ptr += expected_stride;
